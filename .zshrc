...
# alias to sync nvim files from .dotfiles to .config
alias sync_nvim_configs='(function() {
    ...
    find "$src_dir" -type f | while read -r file; do
        ...
        # Handle conflicts and create symlinks
    ...
    find "$dest_dir" -type l | while read -r symlink; do
        ...
        if [[ "$target" != "$src_dir/"* || ! -e "$target" ]]; then
            ...
            if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                ...
                echo "Skipped orphaned symlink: $symlink"
            fi
        fi
    done
